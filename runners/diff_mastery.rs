//! DiffANT Mastery Runner
//!
//! Trains a change detector on 32-dim ternary signal pairs via Runes scripting.
//! Inverse of CompareANT: changed pairs produce positive output, unchanged produce negative.
//! Uses product_features(a, b) as input to the rune.

use std::sync::Arc;
use atomic_neural_transistors::{AtomicNeuralTransistor, Signal, Value};
use atomic_neural_transistors::testdata::{Rng, generate_class_prototypes, add_noise, product_features};

fn main() {
    println!("=== DiffANT Mastery Runner ===\n");
    ...
    for i in 0..n_total {
        ...
        pairs.push(Pair { features, changed });
    }
    ...
    let source = include_str!("../runes/diff_train.rune");
    let mut ant = AtomicNeuralTransistor::from_source(source)
        .expect("Failed to load diff_train.rune");
    ...
    for cycle in 1..=max_cycles {
        ...
        for pair in train_set.iter() {
            // Pack [features(32) | target(1)] = 33 values
            let mut values: Vec<Value> = pair.features.iter()
                .map(|s| Value::Integer(s.current() as i64))
                .collect();
            // Inverted vs compare: changed -> +127, unchanged -> -127
            let target = if pair.changed { 127i64 } else { -127i64 };
        ...
        for pair in test_set.iter() {
            let input: Vec<Value> = pair.features.iter()
                .map(|s| Value::Integer(s.current() as i64))
                .collect();
            ...
            let predicted_changed = output_val > 0;
            if predicted_changed == pair.changed {
                correct += 1;
            }
        }
    ...
    println!("\n=== DiffANT Mastery Runner Complete ===");
}
